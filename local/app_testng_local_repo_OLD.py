...
Apply the 6-step methodology from the training above and provide your analysis in this EXACT format:

### 🎯 Root Cause
[2-3 sentences explaining why the test failed, referencing specific code]
...
```java
// Show the problematic code with fix comment
        ...
        response_text = response_body['content'][0]['text']
               ...
               # Parse markdown format from training (not JSON)
        analysis = {}
        ...
        if root_match:
            ...
            analysis['root_cause'] = response_text[:500]
        
        # Extract What to Check (becomes suggested_fix)
        check_match = re.search(r'### 🔍 What to Check\s*\n(.*?)(?=###|\Z)', response_text, re.DOTALL)
        if check_match:
            analysis['suggested_fix'] = check_match.group(1).strip()
        else:
            analysis['suggested_fix'] = "See full analysis"
        
        # Extract Recommended Fix (becomes prevention_tips)
        fix_match = re.search(r'### 💡 Recommended Fix\s*\n(.*?)(?=###|\Z)', response_text, re.DOTALL)
        if fix_match:
            analysis['prevention_tips'] = fix_match.group(1).strip()
        else:
            analysis['prevention_tips'] = ""
        
        # Extract code snippet
        code_match = re.search(r'```java\s*(.*?)\s*```', response_text, re.DOTALL)
        analysis['code_snippet_fix'] = code_match.group(1).strip() if code_match else ''
        
        # Extract Category
        category_match = re.search(r'Category:\s*([A-Z_]+)', response_text)
        analysis['failure_category'] = category_match.group(1) if category_match else 'UNKNOWN'
        
        # Extract Confidence
        confidence_match = re.search(r'Confidence:\s*(HIGH|MEDIUM|LOW)', response_text, re.IGNORECASE)
        analysis['confidence'] = confidence_match.group(1).lower() if confidence_match else 'medium'
        
        # Extract Time
        time_match = re.search(r'Estimated Fix Time:\s*(\d+(?:-\d+)?)\s*minutes?', response_text)
        analysis['estimated_fix_time'] = f"{time_match.group(1)} minutes" if time_match else '30 minutes'
